...
public class SudokuSolver {
    ...
    private static Stack<Integer> availLocation = new Stack<>();
    ...
    public static Stack<Integer> getAvailLocation(){
        return availLocation;
    }
    public static void printGrid(){
        for (int[] rows : NumberLocation.returnGrid()) {
            for (int value : rows) {
        ...
        for (int[] rows : grid){
            for (int value : rows){
                if (value == 0) {
                    ...
                    availLocation.push(iter);//integers stored in availLocation are at most 80
                }
    ...
    public static void main(String[] args){
        ...
        //fillAvail();
        /*for (int i = 0; i < 9; i++){
            NumberLocation numberLocation = new NumberLocation(i);
            numberLocation.fillLoc(); //will later change fillNISLocations() to fillNotInNLocations()
        }*/
    }
